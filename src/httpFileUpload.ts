...
export const fileStorage = multer.diskStorage({
    destination: (
        request: GetRequestWithUploadParameter,
        _file: Express.Multer.File,
        callback: DestinationCallback
    ): void => {
        let localdir = Config.getConfiguration().filelocation;
        if (request.query.specification !== null) {
            let dir = getSpecificationImageOrDocumentUrl(join(Config.getConfiguration().filelocation, "local"), getFilenameForUpload(request.query.specification!), "");
            if (!fs.existsSync(dir))
                fs.mkdirSync(dir, { recursive: true });
            callback(null, dir)
        }
        else
            callback(new Error("No parameter specification found"), localdir);
    },
    filename: (
        _req: Request,
