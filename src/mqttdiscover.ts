...
export class MqttDiscover {
  ...
  private mqttDiscoveryTopics: Map<string, Map<number, ItopicAndPayloads[]>> =
    new Map<string, Map<number, ItopicAndPayloads[]>>();
  ...
  private generateStateTopic(busid: number, slave: Islave): string {
    return (
      ...
      "/" +
      busid +
      Config.getFileNameFromSlaveId(slave.slaveid) +
      "/state"
    );
  }
  ...
  private generateEntityCommandTopic(
    busid: number,
    slave: Islave,
    ent: Ientity,
  ): string {
  ...
  private generateDiscoveryPayloads(
    ...
    if (slave.specification && this.language)
        ...
        if (e.id >= 0 && !e.variableConfiguration) {
          ...
          if (converter) {
              ...
              if (ent1.variableConfiguration) {
                switch (ent1.variableConfiguration.targetParameter) {
                  ...
                  case VariableTargetParameters.entityUom:
                    if (
                      e.id == ent1.variableConfiguration.entityId &&
                      converter!.getParameterType(e) === "Inumber"
                    ) {
                      obj.unit_of_measurement = (
                        ent1 as ImodbusEntity
                      ).mqttValue;
                    }
                    break;
            ...
            if (!e.variableConfiguration) {
              ...
              if (!obj.unique_id)
                obj.unique_id = "M2M" + busid + filename + e.mqttname;

              if (!obj.value_template)
                obj.value_template = "{{ value_json." + obj.object_id + " }}";
              if (!obj.state_topic)
                obj.state_topic = this.generateStateTopic(busid, slave);
              if (!obj.availability && !obj.availability_topic)
                obj.availability_topic = this.generateAvailatibilityTopic(
                  busid,
                  slave,
                );
              if (!obj.command_topic && !e.readonly)
                obj.command_topic = this.generateEntityCommandTopic(
                  busid,
                  slave,
                  ent,
                );
              switch (converter.getParameterType(e)) {
                case "Iselect":
                  ...
                  if (
                    ...
                    ns &&
                    ns.optionModbusValues &&
                    ns.optionModbusValues.length
                  ) {
                    obj.options = [];
                    ...
                    if (obj.options == undefined || obj.options.length == 0)
                      log.log(
                        LogLevelEnum.warn,
                        "generateDiscoveryPayloads: No options specified for " +
                          obj.name,
                      );
                    else
                      obj.options.forEach((o: IselectOption) => {
                        if (!o)
                          log.log(
                            LogLevelEnum.warn,
                            "generateDiscoveryPayloads: option with no text for " +
                              e.id,
                          );
                      });
                  }
                ...
                case "Inumber":
                  ...
                  if (
                    nn &&
                    nn.device_class &&
                    nn.device_class.toLowerCase() != "none"
                  )
              ...
              payloads.push({
                topic: this.generateEntityConfigurationTopic(
                  busid,
                  slave.slaveid,
                  e,
                ),
      ...
      log.log(
        LogLevelEnum.error,
        "generateDiscoveryPayloads: specification or language is undefined",
      );
    }
    return payloads;
  }
  ...
  private onMqttDiscoverMessage(topic: string, payload: Buffer) {
    ...
    let tp: Map<number, ItopicAndPayloads[]> | undefined =
      this.mqttDiscoveryTopics.get(ids.busSlave);
    if (tp == undefined) {
    ...
    if (payload.length == 0) {
      ...
      if (
        !this.containsTopic({ topic: topic, payload: payload.toString() }, tpy)
      ) {
  ...
  private onMqttCommandMessage(topic: string, payload: Buffer) {
    ...
    if (!bus) {
      log.log(
        LogLevelEnum.error,
        "onMqttCommandMessage: invalid busid " + busid,
      );
      return;
    }
    ...
    if (device && device.specificationid) {
      const spec = ConfigSpecification.getSpecificationByFilename(
        device.specificationid,
      );
      if (spec) {
          ...
          if (cnv) {
            ...
            mr.writeEntityMqtt(
              ...
              .then(() => {
                this.triggerPoll(
                  bus!.getId(),
                  bus!.getSlaveBySlaveId(slaveid)!,
                );
              })
              .catch((e) => {
                log.log(
                  LogLevelEnum.error,
                  "writeEntityMqtt failed: " + e.mesgs,
                );
              });
          }
        }
  ...
  private onMqttMessage(topic: string, payload: Buffer) {
    if (topic.startsWith(Config.getConfiguration().mqttdiscoveryprefix))
      this.onMqttDiscoverMessage(topic, payload);
    ...
    this.mqttDiscoveryTopics.get(key)?.forEach((payload, entity) => {
      payload.forEach((p) => {
        ...
        if (slave && slave.specification)
          ent = (slave.specification as IfileSpecification).entities.find(
            (e) => entity == e.id,
          );
        ...
        if (
          ...
          (ent &&
            p.topic !=
              this.generateEntityConfigurationTopic(
                bus.getId(),
                slave.slaveid,
                ent,
              ))
  ...
  private async publishDiscoveryForSlave(
    bus: Bus,
    slave: Islave,
    spec: ImodbusSpecification,
  ) {
    ...
    if (bus && slave.specification) {
      let discoveryPayloads = this.generateDiscoveryPayloads(
        bus.getId(),
        slave,
        spec,
      );
      for (let tp of discoveryPayloads) {
        ...
        let tpFound = this.mqttDiscoveryTopics
          .get(ids.busSlave)
          ?.get(ids.entityid);
  ...
  private error(msg: any): void {
    let message =
      "MQTT: Can't connect to " +
      Config.getConfiguration().mqttconnect.mqttserverurl +
      " " +
      msg.toString();
  ...
  private connectMqtt(onConnected: () => void, error: (e: any) => void) {
    if (!this.mqttConnectionData) {
      error("No mqtt connection configured.");
      return;
    }

    if (!this.client && this.mqttConnectionData.mqttserverurl) {
      ...
      this.client = connect(
        this.mqttConnectionData.mqttserverurl,
        opts as IClientOptions,
      );
  ...
  validateConnection(callback: (valid: boolean, message: string) => void) {
    if (!this.client)
      this.connectMqtt(
        () => {
          callback(true, "OK");
  ...
  private publishDiscoverySlave(
    ...
    if (bus && slave)
      this.connectMqtt(
        this.publishDiscoveryForSlave.bind(this, bus, slave, spec),
        this.error,
      );
  }
  ...
  private publishState(bus: Bus, slave: Islave, spec: ImodbusSpecification) {
    ...
    if (!slave.specificationid) {
      let msg =
        "Specification not configured for busid: " +
        bus.getId() +
        " slaveid: " +
        slave.slaveid;
      debug(msg);
    ...
    try {
      ...
      this.getMqttClient().publish(
        this.generateAvailatibilityTopic(bus.getId(), slave),
        "online",
      );
    } catch (e) {
      try {
        ...
        Bus.getBusses().forEach((bus) => {
          bus.getSlaves().forEach((slave) => {
            ...
            if (
              pc == undefined ||
              pc >
                (slave.polInterval != undefined
                  ? slave.polInterval / 100
                  : defaultPollCount)
            )
              pc = 0;
            if (pc == 0) {
              ...
              debug(
                "poll: start sending payload busid: " +
                  bus.getId() +
                  " slaveid: " +
                  slave.slaveid,
              );
        ...
        Promise.allSettled(allTopics).then((values) => {
          values.forEach((v) => {
            if (v.status == "rejected")
              log.log(LogLevelEnum.error, v.reason.message);
          });
  ...
  private subscribeDiscovery() {
    ...
    if (this.client == undefined || this.client.disconnected) {
      log.log(LogLevelEnum.error, "subscribeDiscovery: MQTT not connected");
      return;
    }
