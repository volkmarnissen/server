...
export class HttpServer {
  ...
  authenticate(req: Request, res: http.ServerResponse, next: any) {
    ...
    if (
      ...
      (req.url.indexOf("/user/register") >= 0 && pwd && pwd.length)
    ) {
        ...
        switch (HttpServer.validateUserToken(authHeader)) {
          ...
          case MqttValidationResult.tokenExpired:
            ...
            HttpServer.returnResult(
              req,
              res,
              HttpErrorsEnum.ErrUnauthorized,
              "Token expired",
            );
            return;
          default:
            ...
            HttpServer.returnResult(
              req,
              res,
              HttpErrorsEnum.ErrForbidden,
              "Unauthorized (See server log)",
            );
            return;
        }
      }
      ...
      if (config.hassiotoken) {
        ...
        this.configObj.executeHassioGetRequest(
          ...
          () => {
            log.log(
              LogLevelEnum.notice,
              "Supervisor: validate hassio token OK",
            );
            next();
          },
          (e) => {
            log.log(
              LogLevelEnum.error,
              "Supervisor: validate hassio token Failed",
            );
            HttpServer.returnResult(
              req,
              res,
  ...
  init() {
    ...
    let localdir = join(
      Config.getConfiguration().filelocation,
      "local",
      filesUrlPrefix,
    );
    let publicdir = join(
    ...
    this.get(
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        debug(req.url);
        ...
        let a: IUserAuthenticationStatus = {
          registered:
            config.mqttusehassiotoken ||
            (config.username != undefined && config.password != undefined),
          hassiotoken: config.mqttusehassiotoken
            ? config.mqttusehassiotoken
            : false,
          hasAuthToken: authHeader ? true : false,
          authTokenExpired:
            authHeader != undefined &&
            HttpServer.validateUserToken(authHeader) ==
              MqttValidationResult.tokenExpired,
          mqttConfigured: false,
          preSelectedBusId:
            Bus.getBusses().length == 1
              ? Bus.getBusses()[0].getId()
              : undefined,
        };
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        ...
        if (req.query.name && req.query.password) {
          ...
          HttpServer.returnResult(
            ...
            res,
            HttpErrorsEnum.ErrBadRequest,
            JSON.stringify({ result: "Invalid Parameter" }),
          );
        }
      },
    );
    this.get(
      apiUri.specsForSlaveId,
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        debug(req.url);
        ...
        if (msg !== "") {
          ...
          if (bus) {
            bus
              .getAvailableSpecs(
                slaveId,
                req.query.showAllPublicSpecs != undefined,
              )
              .then((result) => {
              ...
              .catch((e) => {
                HttpServer.returnResult(
                  req,
                  res,
                  HttpErrorsEnum.ErrNotFound,
                  "specsForSlaveId: " + e.message,
                );
              });
          }
        }
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        ...
        if (spec && spec.length > 0) {
          HttpServer.returnResult(
            ...
            HttpErrorsEnum.OK,
            JSON.stringify(
              ConfigSpecification.getSpecificationByFilename(spec),
            ),
          );
        } else {
          HttpServer.returnResult(
    ...
    this.get(
      apiUri.specificationFetchPublic,
      (req: Request, res: http.ServerResponse) => {
        debug(req.url);
    ...
    this.get(apiUri.configuration, (req: Request, res: http.ServerResponse) => {
      ...
      try {
        ...
        log.log(
          LogLevelEnum.error,
          "Error getConfiguration: " + JSON.stringify(e),
        );
        HttpServer.returnResult(
          req,
          res,
    ...
    this.get(
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        debug(req.url);
        debug(
          "get specification with modbus data for slave " + req.query.slaveid,
        );
        let msg = this.checkBusidSlaveidParameter(req);
        if (msg !== "") {
          HttpServer.returnResult(req, res, HttpErrorsEnum.ErrBadRequest, msg);
        ...
        if (bus === undefined) {
          HttpServer.returnResult(
            ...
            res,
            HttpErrorsEnum.ErrBadRequest,
            "Bus not found. Id: " + req.query.busid,
          );
          return;
        }
        Modbus.getModbusSpecification(
          ...
          (e: any) => {
            log.log(
              LogLevelEnum.error,
              "http: get /specification " + e.message,
            );
            HttpServer.returnResult(
              req,
              res,
    ...
    this.post(apiUri.validateMqtt, (req: Request, res: http.ServerResponse) => {
      ...
      try {
        if (config.mqttconnect == undefined) {
          this.validateMqttConnectionResult(
            req,
            res,
            false,
    ...
    this.post(
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        debug(
          "POST /specification: " + req.query.busid + "/" + req.query.slaveid,
        );
        ...
        if ((req.query.busid, req.query.slaveid)) {
          ...
          if (bus !== undefined) {
            ...
            if (slaveAddresses)
              testdata =
                M2mSpecification.getEmptyModbusAddressesFromSlaveToTestdata(
                  slaveAddresses,
                );
          }
        }
        if (testdata == undefined) {
          HttpServer.returnResult(
            ...
            res,
            HttpErrorsEnum.ErrBadRequest,
            JSON.stringify("No extended testdata available, try again later"),
          );
          return;
        }
        if (msg !== "") {
        ...
        let originalFilename: string | null = req.query.originalFilename
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        ...
        if (!req.query.spec || req.query.spec.length == 0) {
          HttpServer.returnResult(
            ...
            res,
            HttpErrorsEnum.ErrBadRequest,
            JSON.stringify("pass specification "),
          );
          return;
        }
        let fspec = ConfigSpecification.getSpecificationByFilename(
          req.query.spec,
        );
        if (!fspec) {
          HttpServer.returnResult(
      ...
      (req: GetRequestWithParameter, res: http.ServerResponse) => {
        ...
        if (req.body.slaveid == undefined) {
          HttpServer.returnResult(
            ...
            res,
            HttpErrorsEnum.ErrBadRequest,
            "Bus Id: " + req.query.busid + " Slave Id is not defined",
          );
          return;
        }
    ...
    this.post(
      apiUri.addFilesUrl,
      (req: GetRequestWithUploadParameter, res: http.ServerResponse) => {
        try {
          if (req.query.specification) {
            if (req.body) {
              // req.body.documents
              let config = new ConfigSpecification();
              let files = config.appendSpecificationUrl(
          ...
          if (msg !== "") {
            ...
            if (req.files) {
              ...
              (req.files as Express.Multer.File[])!.forEach((f) => {
                files = config.appendSpecificationFile(
                  req.query.specification!,
                  f.originalname,
                  req.query.usage!,
                );
              ...
              if (files)
                ...
                HttpServer.returnResult(
                  ...
                  res,
                  HttpErrorsEnum.OkNoContent,
                  " specification not found or no files passed",
                );
            } else {
              HttpServer.returnResult(
                req,
    ...
    this.delete(
      ...
      (req: GetRequestWithUploadParameter, res: http.ServerResponse) => {
        if (req.query.specification && req.query.url && req.query.usage) {
          let files = ConfigSpecification.deleteSpecificationFile(
            req.query.specification,
            req.query.url,
            req.query.usage,
          );
