...
import { IdentifiedStates, ImodbusData, ImodbusEntity, Iselect } from '../../src/specification.shared'
 ...
 */
export function setOnEntityNameOrVariableFieldsChangeFunc(valFunc?: (entity: ImodbusEntity) => void) {
  if (valFunc) specificationMethods.copy2Translation = valFunc
  else specificationMethods.copy2Translation = () => {}
}
export function setOnPostModbusEntityFunc(valFunc?: (entity: ImodbusEntity | undefined) => Observable<ImodbusData>) {
  if (valFunc) specificationMethods.postModbusEntity = valFunc
  else
    specificationMethods.postModbusEntity = () => {
      return new Subject<ImodbusData>()
    }
}
