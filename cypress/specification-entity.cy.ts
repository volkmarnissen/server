...
describe('Entity Component tests', () => {
    ...
    cy.get('mat-select[formControlName="variableType"]')
      ...
      .then(() => {
        ...
        setOnEntityNameOrVariableFieldsChangeFunc((entity) => {
          ...
          expect(entity.variableConfiguration?.targetParameter).not.to.be.undefined
          expect((entity as any).name).to.be.undefined
        })
      })
    cy.get('mat-select[formControlName="variableEntity"]').click().get('mat-option').contains('ent 4').click()
    // This ensures, that EntityValidation was called because OnNameValueChange )
    cy.get('mat-select[formControlName="variableEntity"]').should('not.be.null')
  ...
  it('No Variable Type => no variableConfiguration', () => {
    cy.get('mat-select[formControlName="variableType"]').click().get('mat-option').first().click()
  ...
  it('Set Byte Order for Number', () => {
    ...
    cy.get('mat-slide-toggle[formControlName="swapBytes"]').click()
  })
  it('Set Byte Order for Text', () => {
    ...
    cy.openAllExpansionPanels()
    cy.get('[formControlName= "textSwapBytes"]').click()
  })
})
