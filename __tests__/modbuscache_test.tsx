...
let mockedConnectRTU = jest.fn() as jest.MockedFunction<
  typeof ModbusRTU.prototype.connectRTUBuffered
>;

let b34 = [0, 3, 0, 4];
let readHoldingRegistersNormal: (n: number) => Promise<ReadRegisterResult> =
  jest
...
let readHoldingRegistersWithTimeout: (
  n: number,
) => Promise<ReadRegisterResult> = jest
  .fn<(n: number) => Promise<ReadRegisterResult>>()
  .mockResolvedValueOnce(getReadRegisterResult(3).result!)
  .mockResolvedValueOnce(getReadRegisterResult(3).result!)
...
beforeAll(() => {
  ...
  ModbusRTU.prototype.writeRegisters = jest.fn(() =>
    Promise.resolve({ address: 4, length: 1 }),
  );
...
function resultFunction0Illegal(
  addresses: Set<ImodbusAddress>,
  results: ImodbusValues,
): void {
  expect(results.holdingRegisters.get(4)).toBeDefined();
...
describe("submitGetHoldingRegisterRequests", () => {
  ...
  test(
    ...
    (done) => {
      mockMutex.acquire().then(() => {
        ...
        executeSubmitGetHoldingRegisterRequests(
          addresses,
          readHoldingRegistersNormal,
          [resultFunction0, resultFunction1, f],
        );
      });
    },
    10 * 1000,
  ...
  test("submitGetHoldingRegisterRequest with Illegal Address", (done) => {
    mockMutex.acquire().then(() => {
      ...
      let f = (_addresses: Set<ImodbusAddress>, results: ImodbusValues) => {
        expectObjectToBe(
          results.holdingRegisters.get(10),
          getReadRegisterResult(10),
        );
      ...
      executeSubmitGetHoldingRegisterRequests(
        addresses,
        readHoldingRegistersWithIllegalAddress,
        [resultFunction0Illegal, f],
      );
    });
  });
});
