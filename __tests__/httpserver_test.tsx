      ...
      expect(
        response.body.find(
          (specs: IidentificationSpecification) =>
            specs.filename == "waterleveltransmitter",
        ),
      ).not.toBeNull();
      done();
    });
...
test("GET /modbus/specification", (done) => {
  supertest(httpServer.app)
    .get(
      "/api/modbus/specification?busid=0&slaveid=1&spec=waterleveltransmitter",
    )
    .expect(HttpErrorsEnum.OK)
    .then((response) => {
      let spec: ImodbusSpecification = response.body;
      expect(
        ((spec?.entities[0] as ImodbusEntity).mqttValue as number) - 21,
      ).toBeLessThan(0.001);
      done();
    });
});
...
describe("http POST", () => {
  ...
  test("POST /specification: add new Specification rename device.specification", (done) => {
    mWaterlevel.runExclusive(() => {
      ...
      fs.copyFileSync(
        lspec + "waterleveltransmitter.yaml",
        lspec + "waterleveltransmitter.bck",
        undefined,
      );

      let filename =
        yamlDir + "/local/specifications/waterleveltransmitter.yaml";
      fs.unlinkSync(new ConfigSpecification().getSpecificationPath(spec1));
      let url =
        apiUri.specfication +
        "?busid=0&slaveid=2&originalFilename=waterleveltransmitter";
      ...
      supertest(httpServer.app)
        ...
        .then((response) => {
          ...
          supertest(httpServer.app)
            ...
            .then((response) => {
              var found = ConfigSpecification.getSpecificationByFilename(
                spec1.filename,
              )!;
              let newFilename = new ConfigSpecification().getSpecificationPath(
                response.body,
              );
              let foundData = found.testdata.holdingRegisters?.find(
                (data) => data.address == 100 && !data.value,
              );
              expect(foundData).toBeDefined();
              expect(fs.existsSync(newFilename)).toBeTruthy();
              expect(getSpecificationI18nName(found!, "en")).toBe(
      ...
      supertest(httpServer.app)
        ...
        .then((_response) => {
          ...
          expect(
            d.find(
              (ul) =>
                ul.url.endsWith("waterleveltransmitter" + "/" + testPdf) &&
                ul.usage === SpecificationFileUsage.documentation,
            ),
          ).toBeTruthy();
          expect(
            d.find(
              (ul) =>
                ul.url.endsWith("waterleveltransmitter" + "/" + test1) &&
                ul.usage === SpecificationFileUsage.documentation,
            ),
          ).toBeTruthy();
